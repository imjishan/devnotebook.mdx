...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  return (
    <div>
        ...
        {showConfig && (
            <div id="config-panel" className="bg-gray-50 p-6 border border-gray-200 mb-8 rounded-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label htmlFor="target-path" className="block text-xs font-mono text-gray-500 mb-1">Target Folder</label>
                        <input
                            id="target-path"
                    ...
                    <div>
                        <label htmlFor="auth-token" className="block text-xs font-mono text-gray-500 mb-1">Personal Access Token</label>
                        <input
                            id="auth-token"
                            type="password"
        ...
        {status && (
          <div role="status" className={`mb-8 p-4 text-sm font-mono ${status.type === 'success' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
            {status.message}
          </div>
        )}
