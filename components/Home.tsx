      ...
      {posts.map(post => (
        <article key={post.slug} className="group relative">
          <div className="flex flex-col md:flex-row md:items-baseline gap-2 mb-2">
            <h2 className="text-2xl font-bold decoration-2 decoration-gray-300 underline-offset-4">
              <button
                onClick={() => onNavigatePost(post.slug)}
                className="text-left group-hover:underline before:absolute before:inset-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-black rounded-sm"
              >
                {post.title}
              </button>
            </h2>
