    ...
    <div className="space-y-16">
      ...
      {posts.map(post => (
        <article key={post.slug} className="group relative">
          <div className="flex flex-col md:flex-row md:items-baseline gap-2 mb-2">
            <h2 className="text-2xl font-bold group-hover:underline decoration-2 decoration-gray-300 underline-offset-4">
              <button
                ...
                className="text-left before:absolute before:inset-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black focus-visible:ring-offset-2 rounded-sm cursor-pointer"
              >
                {post.title}
              </button>
            </h2>
          ...
          <p id={`desc-${post.slug}`} className="text-gray-600 leading-relaxed max-w-prose">
            {post.description}
          </p>
